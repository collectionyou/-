...
const LLMSettings: React.FC<LLMSettingsProps> = ({ appId, instanceId }) => {
  ...
  const handleLLMTypeChange = (llmType: string) => {
    const nextConfig: Partial<LLMConfig> = { llmType };

    if (llmType === 'deepseek') {
      if (!config.baseUrl) {
        nextConfig.baseUrl = 'https://api.deepseek.com/v1';
      }
      if (!config.model) {
        nextConfig.model = 'deepseek-chat';
      }
    }
  ...
  return (
    <VStack spacing="4" align="start">
      ...
      {selectedOption === 'thirdParty' && (
        <ThirdPartyInterface
          config={config}
          handleLLMTypeChange={handleLLMTypeChange}
