  ...
  <>
    ...
    <HStack mt="4" spacing={4}>
      <Tooltip label="在拼多多平台等平台，是不允许每次重复一个回答的，所以可以插入一个随机符，以规避这个问题">
        <Button onClick={handleInsertRandomChar} colorScheme="teal">
          插入随机符
        </Button>
      </Tooltip>
      <Tooltip label="比如尺码表、商品细节图，都可以作为关键词命中后的图片回复">
        <Button leftIcon={<AttachmentIcon />} onClick={handleInsertImage} colorScheme="pink">
          插入图片
        </Button>
      </Tooltip>
      <Tooltip label="有些无法发送文件或者图片的平台无法使用该文件">
        <Button leftIcon={<AttachmentIcon />} onClick={handleInsertFile} colorScheme="orange">
          插入文件
        </Button>
      </Tooltip>
    </HStack>
