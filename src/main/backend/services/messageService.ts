...
export class MessageService {
  ...
  public async matchKeyword(
    ...
    if (foundKeywordObj) {
      ...
      return this.normalizeReplyContent(chosenReply);
    }
  ...
  private normalizeReplyContent(reply: string): ReplyDTO {
    if (reply.includes('[@]') && reply.includes('[/@]')) {
      const fileStart = reply.indexOf('[@]') + 3;
      const fileEnd = reply.indexOf('[/@]');
      const filePath = reply.substring(fileStart, fileEnd).trim();
      ...
      return {
        type: (isImage ? 'IMAGE' : 'FILE') as MessageType,
        content: filePath,
      };
    }
