...
export class DispatchService {
  ...
  public registerHandlers(socket: socketIo.Socket): void {
    ...
    socket.on('messageService-getMessages', async (data, callback) => {
      ...
      if (reply.type === 'TRANSFER') {
        this.notifyManualTakeover(ctxMap, msgs);
      }
  ...
  private notifyManualTakeover(ctx: Map<string, string>, msgs: any[]): void {
    const lastUserMsg = msgs
      ?.slice()
      ?.reverse()
      ?.find((msg) => msg?.role === 'OTHER' && msg?.content);

    const appName = ctx.get('CTX_APP_NAME');
    const title = appName ? `需要人工接管 - ${appName}` : '需要人工接管';
    const body = lastUserMsg?.content
      ? `未匹配到关键词: ${String(lastUserMsg.content).slice(0, 80)}`
      : '未匹配到关键词，请人工查看当前会话。';

    try {
      shell.beep();
    } catch (error) {
      this.log.warn(`人工接管提示音触发失败: ${String(error)}`);
    }
    ...
    try {
      ...
      this.log.warn(`人工接管通知触发失败: ${String(error)}`);
    }

    this.log.warn(`需要人工接管: ${body}`);
  }
