...
const setupIpcHandlers = (
  ...
  ipcMain.on('open-directory', async (_event, args) => {
    shell.openPath(args);
  });
  ...
  ipcMain.on('electron-store-remove', async (_event, key) => {
    store.delete(key);
  });
  ...
  ipcMain.on('open-url', async (_event, url) => {
  ...
  ipcMain.on('notification', async (_event, title, message) => {
    new Notification({
      title,
      body: message,
    }).show();
  ...
  ipcMain.on(
    ...
    async (_event, { appId, instanceId, tab } = {}) => {
      const args = [];
  ...
  ipcMain.on('open-dataview-window', async (_event, args) => {
